...
int main() {
  ...
  Model robot(state_num, ctl_num);
  Eigen::VectorXd init_state(state_num);
  init_state << 0.0, 0.0, M_PI / 4, 0.0, 0.0; // x, y, theta, lin_vel, ang_vel
  robot.init(init_state);
  ...
  std::vector<Eigen::MatrixXd> weights = predictor.buildWeightMatrix(
      state_weight, input_weight, final_state_weight);
  ...
  for (int i = 0; i < 200; i++) {
    ...
    for (int k = 0; k < pred_horizon; ++k) {
      ...
      Model::model continuous_model =
          robot.getLinearizedModel(ref_point.segment(k * state_num, state_num));
      Model::model discrete_model =
          robot.getDiscretizedModel(continuous_model, 5, dt);
    ...
    Eigen::VectorXd error = (pred_trans_matrix * state) - ref_point;
    for (int k = 0; k < pred_horizon; ++k) {
