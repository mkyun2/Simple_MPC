...
void Model::discretizeModel(int n, double dt) {
  Eigen::MatrixXd I = Eigen::MatrixXd::Identity(transition_matrix_.rows(),
                                                transition_matrix_.cols());
  ...
  Eigen::MatrixXd discrete_input_matrix =
      Eigen::MatrixXd(input_matrix_.rows(), input_matrix_.cols());
  discrete_input_matrix *= 0;

  for (int i = 1; i <= n; i++) {
    discrete_input_matrix += dt * i / factorial(i) *
                             powMatrix(transition_matrix_, i - 1) *
                             input_matrix_;
  }
